...
def parseCommit(commitLines):
    ...
    for nextLine in commitLines:
        if nextLine == '' or nextLine == '\n':
            ...
            # Merge: xxxx xxxx
            m = re.compile('Merge: (.*)').match(nextLine)
            commit['merge'] = m.group(1)
...
if __name__ == '__main__':
    ...
    # print commits
    print( 'Author'.ljust(15) + '  ' + 'Email'.ljust(20) +'  ' + 'Hash'.ljust(8) + '  ' + 'Merge'.ljust(8) + '  ' + 'Date'.ljust(19) + '  ' + 'Message'.ljust(20))
    print( "========================================================================================")
    for commit in commits:
        # print(commit)

        # if merge hash exists
        if "merge" in commit.keys():
            print( commit['author'].ljust(15) + '  ' + commit['email'][:20].ljust(20) + '  ' + commit['hash'][:7].ljust(8) + '  ' + commit['merge'][:7].ljust(8) + '  ' + commit['date'][:18].ljust(19) + '  ' + commit['message'])
        else:
            print( commit['author'].ljust(15) + '  ' + commit['email'][:20].ljust(20) + '  ' + commit['hash'][:7].ljust(8) + '  ' + '       ' + '  ' + commit['date'][:18].ljust(19) + '  ' + commit['message'])
